...
if (empty($_POST['name'])) {
	...
	$rol = $_POST["roleuser"];
	$agenciauser = $_POST["agenciauser"];

	$is_admin = 0;

	$sqlcon = "SELECT * FROM user WHERE cedula = '" . $cedula . "';";
	$query_check_user_name = mysqli_query($con, $sqlcon);
	$query_check_user = mysqli_num_rows($query_check_user_name);
	if ($query_check_user >= 1) {
		$errors[] = "Lo sentimos , la cedula de usuario ya está en uso.";
	} else {
		...
		$sql = "INSERT INTO user ( name, password, cedula, profile_pic, is_active, created_at, rol, agenciauser) VALUES ('$end_name','$password','$cedula','$profile_pic',$status,'$created_at', '$rol' , '$agenciauser')";
