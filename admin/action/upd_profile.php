<?php
	...
	if(isset($_POST['token'])){
		$update=mysqli_query($con,"UPDATE user set name=\"$name\",cedula=\"$cedula\" where id=$id");
		if ($update) {
			$success=sha1(md5("datos actualizados"));
            header("location: ../inicioadmin.php?success=$success");
	   	}else{
	   		// echo "error";
	   	}
			...
			if($_POST['new_password']==$_POST['confirm_new_password']){
				...
				if ($p==$password){ //comprobamos que la contraseña sea igual ala anterior
					...
					if ($update_passwd) {
            			...
            			header("location: ../inicioadmin.php?success_pass=$success_pass");
					}
				}else{
					$invalid=sha1(md5("la contrasena no coincide la contraseña con la anterior"));
            		header("location: ../inicioadmin.php?invalid=$invalid");
				}
			}else{
