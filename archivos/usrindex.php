...
// $row3 = mysqli_fetch_array($trtick);
	...
	<div class="container">
		...
		<form method="post" id="updti" name="updti" enctype='multipart/form-data'>
			...
			<div class="col-md-9 col-sm-9 col-xs-12" hidden>
				<input type="text" name="pedido_id" id="mod_pedido_id" class="form-control" maxlength="10" value="<?php echo $row['5']; ?>"></option>
			</div>
						...
						?>
								...
								<tr>
									...
									<td><a target="_blank" title='Ver documento' href="upload/<?php echo $fila[1]; ?>" class="btn btn-secondary"><i class="fa fa-download"></i>Ver documento</a>
										<a class="btn btn-danger" title='Borrar Documento' href="../archivos/usrborrar.php?id=<?php echo  $fila[0]; ?>&idti=<?php echo  $idti; ?> "><i class="glyphicon glyphicon-trash"></i> Borrar </a>
										<!--  -->
									</td>
...
<script>
	...
	$("#updti").submit(function(event) {
		...
		$.ajax({
			type: "POST",
			url: "../usuario/action/updticket.php",
			data: parametros,
